...
#define WATARA_PSRAM_UNCACHED_BASE ((uintptr_t)0x15000000u)
...
static size_t psram_detect_size(void) {
    volatile uint32_t *base = (volatile uint32_t *)WATARA_PSRAM_UNCACHED_BASE;
    ...
    for (unsigned i = 0; i < sizeof(boundaries) / sizeof(boundaries[0]); ++i) {
        ...
        volatile uint32_t *probe =
            (volatile uint32_t *)(WATARA_PSRAM_UNCACHED_BASE + bytes);
        old0 = base[0];
...
bool __no_inline_not_in_flash_func(watara_psram_init)(void) {
    const bool rp2350a =
